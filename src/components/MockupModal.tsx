...
interface MockupModalProps {
  ...
  onClose: () => void
  onNext: (remainingFiles: ImageFile[]) => void
}
...
export default function MockupModal({
  ...
  const handleNext = () => {
    const remainingFiles = mockupFiles.filter((file) => !filesToDelete.has(file.id))
    onNext(remainingFiles)
  }

  const remainingCount = mockupFiles.length - filesToDelete.size
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg shadow-2xl w-full max-w-7xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-700">
          <div>
            <h2 className="text-2xl font-bold text-white">Review Mockups</h2>
            <p className="text-gray-400 text-sm mt-1">
              {mockupFiles.length === 0
                ? 'No mockups loaded'
                : `${mockupFiles.length} mockup${mockupFiles.length !== 1 ? 's' : ''} • ${remainingCount} selected`}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {!hasImages ? (
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[400px] text-gray-400">
              ...
              <p className="text-xl font-semibold mb-2">No mockups loaded</p>
              <p className="text-sm">Please upload or load images first</p>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 p-2">
                      ...
                      <p className="text-gray-400 text-xs">
                        {file.isFromDatabase ? 'Saved' : 'New'} • {file.source}
                      </p>
                    </div>
                  </div>
                )
        ...
        <div className="p-6 border-t border-gray-700 flex justify-between items-center">
          <div className="text-sm text-gray-400">
            {filesToDelete.size > 0 ? (
              <span className="text-red-400">
                {filesToDelete.size} image{filesToDelete.size !== 1 ? 's' : ''} marked for removal
              </span>
            ) : (
              <span>Click × on any image to mark it for removal</span>
            )}
          </div>
          <div className="flex gap-3">
            ...
            <button
              onClick={handleNext}
              disabled={!hasImages || remainingCount === 0}
